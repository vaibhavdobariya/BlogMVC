...
namespace BlogMVC.Controllers
{
    public class AdminTagController : Controller
    {
        ...
        {
            var otag = blogDbContext.Tags.Find(id);
            if (otag != null)
            {
                blogDbContext.Tags.Remove(otag); // Remove the tag
                blogDbContext.SaveChanges();    // Commit changes to the database
                return RedirectToAction("List"); // Redirect to the list view
                                                 // Redirect if the tag doesn't exist
            }
            return RedirectToAction("List");
        }
